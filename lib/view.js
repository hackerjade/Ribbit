(function() {
  var Ribbit = window.Ribbit = window.Ribbit || {};

  var View = Ribbit.View = function(game, ctx) {
    this.game = game;
    this.ctx = ctx;
    Ribbit.Util.loadSprites();
  };

  View.MOVES = {
    "up":   [0, -1],
    "down": [0, 1],
    "left": [-1, 0],
    "right": [1, 0]
  };

  View.prototype.greeting = function (welcome) {
    this.bindKeyHandlers(welcome, true);
    var that = this;
    this.interval = setInterval(function () { welcome.draw(that.ctx); }, 60);
  };

  View.prototype.start = function (game) {
    clearInterval(this.interval);
    this.bindKeyHandlers();
    this.game = game;
    var that = this;

    this.interval = setInterval(function () {
      that.game.step();
      that.game.draw(that.ctx);
    }, 60);
  };

  View.prototype.bindKeyHandlers = function (welcome, restrict) {
    var game = welcome || this.game;

    Object.keys(View.MOVES).forEach(function(k) {
      var dir = {};
      dir[k] =  View.MOVES[k];
      key(k, function(e) { game.frog.leap(dir, restrict); });
    });
  };
})();
