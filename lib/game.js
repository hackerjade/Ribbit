(function() {
  ...
  Game.WIDTH = 1155;
  Game.HEIGHT = 800;
  Game.LILIES = [80, 325, 570, 815, 1060];
  Game.VEHICLE_SETUP = [
  ...
  Game.prototype.checkFrogStatus = function () {
    ...
    if (this.frogOnLilypad()) { this.updateScore(); }
  };
  ...
  Game.prototype.frogOnLily = function (idx) {
    ...
    lily = Game.LILIES[idx];
    var lilyLeft = lily - 60, lilyRight = lily + 60;
  ...
  //
  // Game.prototype.frogMissedLily = function(){
  ...
  Game.prototype.drawSafeFrogs = function (ctx) {
    this.safeFrogs.forEach(function(idx) {
      var x = Game.LILIES[idx] - 20;
      ctx.drawImage(Ribbit.Util.happySprite, 0, 0, 32, 32, x, 21, 37, 37);
    });
  };
  ...
  Game.prototype.drawLilies = function (ctx) {
    this.lilies.forEach(function (i) {
      ...
      [[0.75, 1.95], [1.25, 0.45]].forEach(function(el) {
        ctx.beginPath();
        ctx.arc(x, 40, 15, el[0] * Math.PI, el[1] * Math.PI, false);
