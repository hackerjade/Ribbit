(function() {
  ...
  Frog.DIRECTIONS = {
    up:   [12, 369, 23, 17, 33, 27],
    down: [80, 369, 23, 17, 33, 27],
    left: [80, 335, 19, 23, 29, 33],
    right:[12, 335, 19, 23, 29, 33]
  };

  Frog.prototype.positionFrog = function () {
    this.width = Frog.DIRECTIONS[this.facing][4] + this.delta;
    this.pos = [this.dim_x - this.width + this.delta, this.dim_y];
  };
  ...
  Frog.prototype.CheckBounds = function (direction) {
    if (this.row - direction[1] > -1) { this.row -= direction[1]; }
  ...
  Frog.prototype.drawSkull = function () {
    if (this.dead) {
      ...
      ctx.drawImage(Ribbit.Util.deadSprite, 4, 2, 19, 24, x, y, 29, 34);
      return true;
    }
    return false;
  };
  ...
  Frog.prototype.hitByCar = function (car) {
    ...
    var frogLeft = this.pos[0], frogRight = frogLeft + this.width;
    return this.touchingCar(frogRight, car) || this.touchingCar(frogLeft, car);
  };
